...
class ResultController extends Controller
{
    ...
    public function ajaxResults(Request $request) {
        ...
        if(count($foundTopic) > 0 && count($foundSection) > 0 ){
            ...
            ->whereRaw("progresses.created_at = to_timestamp(select min(`created_at`) from progresses where user_id = users.id AND section_id = '$section_id') OR (progresses.score is NULL AND enrolls.topic_id='$topic_id')")->get();
